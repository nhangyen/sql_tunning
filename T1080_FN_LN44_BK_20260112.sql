FUNCTION     T1080_FN_LN44_BK_20260112 (p_Branch    Varchar2
                                         ,p_Tungay Date
                                         ,p_Denngay   Date
                                         ,p_So_Tk     Varchar2)
...
Begin
  Open v_Cursor For
   select d.brn,c.branch_code,i.sub_branch_name,to_char(d.effdt,'dd/MM/yyyy') NGAY,c.customer_id MAKH,d.accno, nvl(t.du_cuoi_vnd,0) DUNO, d.CURRSTAT TTN_TRUOC_DC,
    ...
    d.maker_Id NGUOI_TH
    from VNDKLB.CLVWS_MANUAL_STATUS_CHANGE@fcpdb d
    inner join vndklb.CLTB_ACCOUNT_APPS_MASTER@fcpdb c on c.account_number = d.accno
    inner join ods.it_main_sub i on i.sub_branch = c.branch_code
    left join
    (
      select  h.account_number,sum(amount_due)-sum(amount_settled) du_cuoi_vnd from vndklb.CLTB_ACCOUNT_schedules@fcpdb h
  ...
  Open v_Cursor For
   select d.brn,c.branch_code,i.sub_branch_name,to_char(d.effdt,'dd/MM/yyyy') NGAY,c.customer_id MAKH,d.accno, nvl(t.du_cuoi_vnd,0) DUNO, d.CURRSTAT TTN_TRUOC_DC,
    ...
    inner join vndklb.CLTB_ACCOUNT_APPS_MASTER @fcpdb c on c.account_number = d.accno
    inner join ods.it_main_sub i on i.sub_branch = c.branch_code
    left join tr_ku t on t.so_tk_vay = d.accno and t.transaction_date = p_Denngay
    where  d.effdt between p_Tungay and p_Denngay
    and (p_So_Tk  is null OR p_So_Tk  = 'ALL' OR d.accno = p_So_Tk)
    and (instr(it_fn_get_str_br(P_BRANCH), d.brn) > 0);
  Return v_Cursor;
